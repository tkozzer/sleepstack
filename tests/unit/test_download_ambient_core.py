...
class TestDownloadAndProcessAmbientSound:
    ...
    @patch('uuid.uuid4')
    def test_download_and_process_default_assets_dir(self, mock_uuid, mock_process, mock_download, mock_get_info, mock_validate):
            ...
            with patch('pathlib.Path.mkdir'):
                with patch('pathlib.Path.exists') as mock_exists:
                    ...
                    with patch('pathlib.Path.parent') as mock_parent:
                        mock_parent.return_value.glob.return_value = [Path('/tmp/sleepstack_download_test123.mp3')]
    ...
    @patch('sleepstack.download_ambient.process_audio')
    @patch('uuid.uuid4')
    def test_download_and_process_short_video_error(self, mock_uuid, mock_process, mock_download, mock_get_info, mock_validate):
        """Test download_and_process_ambient_sound with short video."""
        # Mock video info with short duration
        mock_get_info.return_value = {'duration': 30}  # 30 seconds, less than 60
        
        # Mock file operations to prevent the "No audio file was downloaded" error
        with patch('tempfile.gettempdir', return_value='/tmp'):
            with patch('pathlib.Path.mkdir'):
    ...
    @patch('sleepstack.download_ambient.process_audio')
    @patch('uuid.uuid4')
    def test_download_and_process_no_downloaded_file(self, mock_uuid, mock_process, mock_download, mock_get_info, mock_validate):
...
class TestMain:
    ...
    @patch('sys.argv', ['download_ambient.py', 'https://www.youtube.com/watch?v=test', 'test_sound'])
    ...
    def test_main_success(self, mock_exit, mock_download):
        ...
        mock_print.assert_called_once_with("Successfully downloaded and processed: /path/to/test_sound_1m.wav")
    ...
    def test_main_wrong_args(self, mock_exit):
        ...
        mock_print.assert_called_once_with("Usage: python download_ambient.py <youtube_url> <sound_name>")
